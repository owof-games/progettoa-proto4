...
namespace AutoLetterbox
{
    ...
    public static class Util
    {
        ...
        public static float BezierCurve(float[] p, float t)
        {
            if (p.Length > 2)
            {
                var newPoints = new float[p.Length - 1];
                for (var i = 0; i < newPoints.Length; i++) newPoints[i] = Lerp(p[i], p[i + 1], t);
        ...
        public static Vector3 BezierCurve(Vector3[] p, float t)
        {
            if (p.Length > 2)
            {
                var newPoints = new Vector3[p.Length - 1];
                for (var i = 0; i < newPoints.Length; i++) newPoints[i] = Lerp(p[i], p[i + 1], t);
        ...
        {
            var hitDirection = -normalOfCollision;
            var newDirection = originalVelocity.normalized;

            newDirection = new Vector3(newDirection.x - hitDirection.x, newDirection.y - hitDirection.y, 0);
            if (newDirection.sqrMagnitude == 0f) newDirection = -originalVelocity.normalized;
        ...
        {
            if (sender != null)
                Debug.Log(sender.GetType() + ": " + error);
            else
        ...
        public static void NullError(object sender, string variableName, string extraNotes = "")
        {
            if (sender == null)
                ...
                Debug.Log(sender.GetType() + ": " + variableName + " is null! " + extraNotes);
        }
        ...
        /// <returns></returns>
        public static bool RectContainsRect(Vector2 extremeMinA, Vector2 extremeMaxA, Vector2 extremeMinB,
            Vector2 extremeMaxB)
        {
        ...
        {
            var deg = Mathf.Atan2(_vector.y, _vector.x) * Mathf.Rad2Deg;
        ...
        public static bool IsPointOnMainCamera(Vector3 _point)
        {
            ...
            var screenPoint = Camera.main.WorldToScreenPoint(_point);
