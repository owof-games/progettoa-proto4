...
namespace FMODUnity
{
    public class EditorUtils : MonoBehaviour
    {
        ...
        private static List<Bank> loadedPreviewBanks = new List<Bank>();
        ...
        private static List<EventInstance> previewEventInstances = new List<EventInstance>();
        ...
        private static void CreateSystem()
        {
            ...
            RESULT result = Debug.Initialize(DEBUG_FLAGS.LOG, DEBUG_MODE.FILE, null, "fmod_editor.log");
            if (result != RESULT.OK)
            {
            ...
            {
                ADVANCEDSETTINGS studioAdvancedSettings = new ADVANCEDSETTINGS();
        ...
        public static void OpenOnlineDocumentation(string section, string page = null, string anchor = null)
        {
            ...
            string version = string.Format("{0:X}.{1:X}", VERSION.number >> 16, (VERSION.number >> 8) & 0xFF);
            string url;
        ...
        public static void LoadPreviewBanks()
        {
            ...
            {
                Bank previewBank;
                RESULT result = System.loadBankFile(bank.Path, LOAD_BANK_FLAGS.NORMAL, out previewBank);
                if (result != RESULT
            ...
            float volume = 1, float startTime = 0.0f)
        {
            ...
            foreach (EditorParamRef param in eventRef.Parameters)
            {
                ...
                float value = previewParamValues.ContainsKey(param.Name)
                    ? previewParamValues[param.Name]
                    : param.Default;
        ...
        {
            foreach (EventInstance eventInstance in previewEventInstances)
            {
