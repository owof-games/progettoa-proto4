...
namespace FMODUnity
{
    ...
    public class RuntimeManager : MonoBehaviour
    {
...
#if UNITY_EDITOR
        private List<EventInstance> eventPositionWarnings = new List<EventInstance>();
#endif
        ...
        private static RESULT DEBUG_CALLBACK(DEBUG_FLAGS flags, IntPtr filePtr, int line, IntPtr funcPtr,
            IntPtr messagePtr)
        {
            StringWrapper file = new StringWrapper(filePtr);
            StringWrapper func = new StringWrapper(funcPtr);
            StringWrapper message = new StringWrapper(messagePtr);
        ...
        private RESULT Initialize()
        {
            ...
            OUTPUTTYPE outputType = currentPlatform.GetOutputType();
            ...
            debugCallback = new DEBUG_CALLBACK(DEBUG_CALLBACK);
            result = FMOD.Debug.Initialize(fmodSettings.LoggingLevel, DEBUG_MODE.CALLBACK, debugCallback, null);
            ...
            {
                studioInitFlags |= INITFLAGS.MEMORY_TRACKING;
            }
        ...
        private static void SetThreadAffinities(Platform platform)
        {
            foreach (ThreadAffinityGroup group in platform.ThreadAffinities)
            {
                ...
                {
                    THREAD_TYPE fmodThread = RuntimeUtils.ToFMODThreadType(thread);
                    THREAD_AFFINITY fmodAffinity = RuntimeUtils.ToFMODThreadAffinity(group.affinity);
                        ...
                        else
                        {
                            ...
                            if (Time.deltaTime != 0)
                            {
                                ...
                                velocity = Vector3.ClampMagnitude(velocity,
                                    20.0f); // Stops pitch fluttering when moving too quickly
                            }

                            attachedInstances[i].lastFramePosition = position;
                            attachedInstances[i].instance
                                .set3DAttributes(RuntimeUtils.To3DAttributes(attachedInstances[i].transform, velocity));
                        }
                    }
                }
        ...
        private static AttachedInstance FindOrAddAttachedInstance(EventInstance instance, Transform transform,
        ...
        {
            AttachedInstance attachedInstance = FindOrAddAttachedInstance(instance, gameObject.transform,
        ...
        {
            AttachedInstance attachedInstance =
                FindOrAddAttachedInstance(instance, transform, RuntimeUtils.To3DAttributes(transform));
        ...
        {
            AttachedInstance attachedInstance = FindOrAddAttachedInstance(instance, transform,
                RuntimeUtils.To3DAttributes(transform, rigidBody));
        ...
        {
            AttachedInstance attachedInstance = FindOrAddAttachedInstance(instance, transform,
                RuntimeUtils.To3DAttributes(transform, rigidBody2D));
 ...
 false, System.Action completionCallback = null)
        ...
        {
            Bus bus;
            if (StudioSystem.getBus(path, out bus) != RESULT.OK)
            {
        ...
        {
            VCA vca;
            if (StudioSystem.getVCA(path, out vca) != RESULT.OK)
            {
        ...
        private void SetOverlayPosition()
        {
            ...
            switch (currentPlatform.OverlayRect)
            {
                ...
                case ScreenPosition.BottomCenter:
                    windowRect = new Rect((Screen.width / 2) - (width / 2), Screen.height - (height + margin), width,
                        height);
                    break;
                case ScreenPosition.BottomRight:
                    windowRect = new Rect(Screen.width - (width + margin), Screen.height - (height + margin), width,
                        height);
                    break;
                case ScreenPosition.Center:
                    windowRect = new Rect((Screen.width / 2) - (width / 2), (Screen.height / 2) - (height / 2), width,
                        height);
                    break;
 ...
 Camera.main.GetComponent<UniversalAdditionalCameraData>();
                    if (fmodOverlayLayer != -1 && mainCameraData != null)
                    {
                        ...
                        vrDebugCamera.GetComponent<UniversalAdditionalCameraData>().renderType =
...
#else
                    RuntimeUtils.DebugLogWarning(
                        "[FMOD] UNITY_URP_EXIST is not defined. The VR debug overlay requires the Universal Render Pipeline.");
                    break;
#endif
                default:
                    windowRect = new Rect(margin, margin, width, height);
